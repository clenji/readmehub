...
const readmeStructure = [
  ...
  },
  {
    ...
    label: '双语',
    note: '默认 README.md 中文，README.en.md 英文',
    Icon: Languages,
  },
  {
    title: 'What is this',
...
function toCommandBlocks(value: string, language: ProjectLanguage = '中文') {
  const commands = splitLines(value)
  if (!commands.length) {
    const comment = language === 'English' ? '# Add commands here' : '# 待补充命令'
    return `\`\`\`bash\n${comment}\n\`\`\``
  }
...
function readmeFileName(language: ProjectLanguage) {
  return language === 'English' ? 'README.en.md' : 'README.md'
}

function buildLanguageLinks(language: ProjectLanguage) {
  return language === 'English'
    ? '[Chinese](README.md) · [English](README.en.md)'
    : '[中文](README.md) · [English](README.en.md)'
}

function localizedStatus(status: ProjectStatus, language: ProjectLanguage) {
  if (language === '中文') {
    return status
  }
...
function localizedReadmeLanguage(language: ProjectLanguage) {
  return language === 'English' ? 'English (README.en.md)' : '中文（默认 README.md）'
}
...
function buildCloneCommand(project: ProjectInput, name: string, language: ProjectLanguage) {
...
function buildAgentUsage(name: string, language: ProjectLanguage) {
  if (language === 'English') {
    return `## AI Usage

When asking Codex, Claude Code, Cursor Agent, or a similar code agent to work on this project, use this handoff prompt:

\`\`\`text
Read this project's README files, config files, script entry points, docs, and tests first. Then help me maintain ${name} based on the actual code facts.

Requirements:
- Start by summarizing the project positioning, stack, main entry points, and run commands you found.
- Check git status before editing so you do not overwrite my existing changes.
- Follow the current project structure and naming before adding new abstractions.
- Do not commit secrets, personal data, runtime state, or local generated files.
- After editing, run the smallest relevant project checks and report the result.
\`\`\`

Good AI tasks:

- Improve README, docs, examples, and run instructions.
- Add narrow regression tests based on existing test entry points.
- Clarify project structure, risk boundaries, and maintenance notes.
- Fix issues with clear reproduction steps.

Tasks that need human confirmation:

- Delete data, migrate production state, or overwrite private local config.
- Publish releases, push to remotes, change License, or disclose sensitive information.
- Make judgments that require the project owner, a doctor, lawyer, or financial professional.
`
  }
...
function buildStarHistory(repoUrl: string, language: ProjectLanguage) {
  const repoSlug = githubRepoSlug(repoUrl)

  if (!repoSlug) {
    const fallback =
      language === 'English'
        ? 'Add the GitHub repository URL to show the star growth chart.'
...
function buildLinks(project: ProjectInput, language: ProjectLanguage) {
  const links = [
  ...
  return language === 'English'
    ? '[GitHub](https://github.com/) · [Documentation](#docs)'
    : '[GitHub](https://github.com/) · [Documentation](#文档)'
}

function buildNav(project: ProjectInput, language: ProjectLanguage) {
  const defaultEnglishNav = [
    'Features',
    'Quick Start',
    'Screenshots',
    'AI Usage',
  ...
  const currentNav = splitLines(project.navItems)
  const usesInitialNav = project.navItems.trim() === initialProject.navItems.trim()
  const navItems = language === 'English' && usesInitialNav ? defaultEnglishNav : currentNav

  if (!navItems.length) {
    return language === 'English'
      ? '[Features](#features) · [Quick Start](#quick-start) · [Contributing](#contributing) · [License](#license) · [Star History](#star-history)'
      : '[功能](#功能) · [快速开始](#快速开始) · [贡献](#贡献) · [License](#license) · [Star History](#star-history)'
  }
...
function buildGeneratedReadme(project: ProjectInput) {
  ...
  if (project.language === 'English') {
    const comparisonRows = [
      [
        'Source of truth',
        competitors[0] || 'Template or blank docs',
        advantages[0] || 'Organized from project facts',
      ],
      [
        'Quick start',
        competitors[1] || 'Readers need to guess the setup path',
        advantages[1] || 'Install, run, and docs are grouped together',
      ],
      [
        'Community entry',
        competitors[2] || 'Contribution path is hard to find',
        advantages[2] || 'Contribution and issue paths are visible',
      ],
    ]
...
${buildLanguageLinks(project.language)}

${compact(project.tagline, 'Explain in one sentence what this project helps people do.')}

${buildLinks(project, project.language)}

${buildNav(project, project.language)}

## Positioning

${compact(
  project.description,
  'Use 2 to 4 sentences to explain what the project is, who it serves, and what problem it solves.',
)}

| Status | Audience | Main Use Cases | README Language |
| --- | --- | --- | --- |
| ${localizedStatus(project.status, project.language)} | ${compact(project.audience, 'To be added')} | ${compact(project.scenario, 'To be added')} | ${localizedReadmeLanguage(project.language)} |

## Features

${toMarkdownList(project.features, 'Add core features')}
...
${buildCloneCommand(project, name, project.language)}

Install dependencies:

${toCommandBlocks(project.installCommand, project.language)}

Start the project:

${toCommandBlocks(project.quickStartCommand, project.language)}

Open the local URL shown in the terminal. If the port is already in use, follow the actual terminal output.

Local checks:

\`\`\`bash
# Run the smallest relevant project checks, such as lint, test, or build
...
${buildAgentUsage(name, project.language)}

## Why ${name}

| Dimension | Common Approach | ${name} |
| --- | --- | --- |
${comparisonRows}

## Docs

- Documentation: ${project.docsUrl ? `[Documentation](${project.docsUrl})` : 'To be added'}
- Demo: ${project.demoUrl ? `[Demo](${project.demoUrl})` : 'To be added'}
- Issues: ${project.repoUrl ? `[Issues](${project.repoUrl}/issues)` : 'To be added'}

## Contributing

${compact(project.contributing, 'Issues and pull requests are welcome. Please describe the problem and expected impact first.')}

Community: ${compact(project.community, 'GitHub Issues / Discussions')}

## License

${compact(project.license, 'MIT')}

${buildStarHistory(project.repoUrl, project.language)}
`
  }
...
| --- | --- | --- | --- |
| ${localizedStatus(project.status, project.language)} | ${compact(project.audience, '待补充')} | ${compact(project.scenario, '待补充')} | ${localizedReadmeLanguage(project.language)} |
...
function buildAudit(project: ProjectInput, generatedReadme: string): AuditItem[] {
  ...
  const bilingualEvidence = /README\.md/.test(reviewText) && /README\.en\.md/.test(reviewText)
  ...
  const audits = [
    ...
    },
    {
      ...
      weight: 1.15,
      evidence: '导航、双语互链、文档、AI 调用、贡献、License、Star 进展图是现代 README 的基础骨架。',
      action: '补齐导航列表、README.md / README.en.md 互链、文档入口、AI 调用说明、贡献规则、License 和 Star History。',
      Icon: BookOpen,
    },
    {
      title: '差异表达',
    ...
    },
    {
      ...
      evidence: hasLanguageRisk
        ...
        : bilingualEvidence
          ? `当前输出 ${readmeFileName(project.language)}，并保留 README.md / README.en.md 互链。`
          : '缺少 README.md / README.en.md 双语互链。',
      action: '默认中文 README.md，英文 README.en.md；两份文档顶部互链，正文保持单一主语言。',
      Icon: Languages,
    },
...
function App() {
  ...
  const outputFileName = readmeFileName(project.language)
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="toolbar" aria-label={`${outputFileName} 操作`}>
          <button type="button" onClick={copyReadme}>
            <Clipboard size={17} aria-hidden="true" />
            {copied ? '已复制' : `复制 ${outputFileName}`}
          </button>
