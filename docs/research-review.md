...
- 后续使用反馈：README 不只给人读，也要让 Codex、Claude Code、Cursor Agent 等 AI agent 能快速理解项目、判断边界并安全接手。
- 双语维护要求：默认中文 `README.md`，英文放 `README.en.md`，两份文档顶部互相链接，避免同一份 README 中混写中英文正文。
...
用户图里的 README 架构是合理的：先建立信任和定位，再给快速开始，再用 GIF 或截图解释核心路径，随后说明差异、AI 调用方式、文档和贡献方式，最后用 Star 进展图展示社区增长轨迹。这个顺序比传统“安装、使用、API”更适合第一次访问，也更适合让 agent 在最短路径里接手项目。

## 反模式

- 商业宣传压过工程事实：很多形容词，但没有安装、运行、贡献路径。
- 中文英文混写：技术词可以保留英文，但正文主语言应该一致。
- 只有单份 README：中文和英文受众被塞进同一份文档，或者英文版缺失互链。
- 过短：只有一句简介和安装命令，读者无法判断边界。
- 过长：把设计文档、教程和博客全部塞进 README，导致 10 分钟内无法理解项目。
- 缺少视觉证据：没有截图、GIF、asciinema 或视频，读者只能靠想象。
- 缺少 AI 调用边界：只告诉人怎么运行，没有告诉 agent 应读哪些文件、可做哪些任务、哪些数据不能碰。
- 缺少社区增长信号：只展示当前 star 数，不展示项目随时间获得关注的趋势。
...
- 内容骨架：把用户图片中的结构转换为可操作的 README 顺序，并加入 AI 调用说明。
- Agent 调用：生成可复制给 Codex、Claude Code、Cursor Agent 的项目接手提示词、适用任务和安全边界。
- 双语版本：默认输出中文 `README.md`，英文版本输出为 `README.en.md`，两份文档顶部保留互链。
- Star 进展图：在 README 底部生成 Star History 图，让项目增长轨迹成为可见社区信号。

## 标准撰写流程

1. 顶部建立识别：项目名、一句话定位、徽章、导航。
2. 准备双语文件：默认中文 `README.md`，英文 `README.en.md`，两份文档顶部互相链接，正文各自保持单一主语言。
3. 写清项目事实：项目状态、目标读者、主要场景、技术栈。
4. 给出 10 分钟上手路径：环境要求、获取代码、安装命令、启动命令、本地访问地址、最小验证、演示截图或 GIF。
5. 写 AI 调用方式：agent 应读取的文件、可执行任务、不可自动决策的风险动作。
6. 说明差异和边界：X vs Y、适合/不适合的场景、运行限制。
7. 补齐维护入口：文档、贡献、Issue、License。
8. 底部放 Star 进展图：使用 GitHub 仓库地址生成 Star History，展示项目随时间获得关注的趋势。
...
| 10 分钟上手 | 高 | 环境要求、获取代码、安装命令、启动命令、本地地址、最小验证、阅读时长 |
| 结构完整 | 高 | 导航、文档、AI 调用、贡献、License、Star 进展图 |
| GitHub 搜索友好 | 中 | 关键词、徽章、仓库链接、文档入口 |
| 图文与演示 | 中 | 截图、GIF、视频、demo 链接 |
| 差异表达 | 中 | X vs Y、替代方案、项目边界 |
| Agent 接手 | 中 | AI 调用提示、可做任务、不可自动执行的风险动作 |
| 社区增长 | 低 | Star History、stargazers 趋势、项目增长轨迹 |
| 社区可信度 | 中 | 贡献说明、问题反馈、维护状态 |
| 语言与双语版本 | 中 | 默认中文 README.md、英文 README.en.md、顶部互链、正文主语言一致 |
