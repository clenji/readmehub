...
2. 写清项目事实：项目状态、目标读者、主要场景、技术栈。
3. 给出 10 分钟上手路径：环境要求、获取代码、安装命令、启动命令、本地访问地址、最小验证、演示截图或 GIF。
4. 写 AI 调用方式：agent 应读取的文件、可执行任务、不可自动决策的风险动作。
5. 说明差异和边界：X vs Y、适合/不适合的场景、运行限制。
6. 补齐维护入口：文档、贡献、Issue、License。
7. 底部放 Star 进展图：使用 GitHub 仓库地址生成 Star History，展示项目随时间获得关注的趋势。
